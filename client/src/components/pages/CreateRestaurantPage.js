...
class CreateRestaurantPage extends Component {
  ...
  createRestaurant(event) {
    ...
    const params = {
      ...
      address: `${formData.get("street_address")}, ${formData.get(
        "city"
      )}, ${formData.get("state")}, ${formData.get("country")}, ${formData.get(
        "zip_code"
      )}`,
    ...
    Restaurant.create(params).then(data => {
      ...
      for (let e of document.querySelectorAll(
        ".AddressAutoComplete .form-control"
      )) {
        e.value = "";
      }
    });
    ...
    return (
      <main className="CreateRestaurantPage">
        ...
        <form onSubmit={this.createRestaurant} className="restaurant_form">
          ...
          <div className="form-group">
            <label className="form-control" htmlFor="imgUrl">
              <h3>Restaurant Logo Url(Optional)</h3>
              <input
                className="form-control"
                type="text"
                name="imgUrl"
                style={{ border: "solid 0.5px rgb(120,120,120)" }}
              />
            </label>
          </div>

          <input
            className="form-control btn btn-outline-success submit"
            type="submit"
